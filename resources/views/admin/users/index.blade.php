    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" id="myTable">
        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr class="header">
                ...
                <th scope="col" class="px-4 py-3 max-w-xs truncate">Remarks</th>              
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                ...
                <td class="px-6 py-4">{{ $item->email }}</td>
                 <td>
                    <form action="{{route('change.email.status')}}" method="post" id="userstatus-{{ $item->id }}">
                        @csrf
                        <div class="flex items-center">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" value="" class="sr-only peer" {{ $item->email_verified_at !== null ? 'checked' : '' }}
                                    onchange="document.getElementById('userstatus-{{ $item->id }}').submit()">
                                <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600 dark:peer-checked:bg-blue-600"></div>
                                <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $item->email_verified_at !== null ? 'Active' : 'Inactive' }}</span>
                            </label>
                        </div>
